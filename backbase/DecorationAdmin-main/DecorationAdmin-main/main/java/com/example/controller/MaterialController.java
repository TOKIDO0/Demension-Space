...
public class MaterialController {
    ...
    public Result<?> findPage(@RequestParam(required = false, defaultValue = "") String name,
        ...
        if (StrUtil.isNotBlank(name)) {
            query.like(Material::getName, name);
        }
    ...
    public void export(HttpServletResponse response) throws IOException {
        ...
        for (Material obj : all) {
            ...
            row.put("所属类别", obj.getClassname());
            row.put("材料代码", obj.getId());
            row.put("材料名", obj.getName());
            row.put("用途", obj.getUsages());
        ...
        String fileName = URLEncoder.encode("材料信息", "UTF-8");
    ...
    public Result<?> upload(@PathVariable String fileId) {
        ...
        for (List<Object> row : lists) {
            ...
            obj.setUsages((String) row.get(3));
