...
public class UserController {
    ...
    @PostMapping("/reset")
    public Result<User> reset(@RequestBody User user, HttpServletRequest request){
        User res = userService.reset(user);
        request.getSession().setAttribute("user", user);
        logService.log(StrUtil.format("用户 {} 修改密码成功", user.getUsername()));
        return Result.success(res);
    }

    @PostMapping("/changePassword")
    public Result<User> changePassword(@RequestBody User user,HttpServletRequest request){
        User res = userService.changePassword(user);
        request.getSession().setAttribute("user",user);
        logService.log(StrUtil.format("用户 {} 修改密码成功",user.getUsername()));
        return Result.success(res);
    }
    ...
    public Result<?> changePwd(@RequestBody User user) {
        logService.log(StrUtil.format("修改密码：{} ", user.getUsername()));
