...
async function supabaseGetUser(url, serviceKey, emailLower, emailOrig) {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!emailN || !password) return res.status(400).json({ error: 'invalid_input' })
    ...
    if (supabaseUrl && supabaseServiceKey && jwtSecret) {
      const row = await supabaseGetUser(supabaseUrl, supabaseServiceKey, emailN, email)
